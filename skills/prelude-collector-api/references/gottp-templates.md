...
## Using TTP Templates in API Curl

The `cli-template` field in a mapping is a plain string (NOT JSON-encoded like `cli-commands`):
...
curl -sk -X POST "$BASE_URL/api/v1/models/$MODEL_ID/mappings" \
  ...
  -d '{
    ...
    "cli-template": "<group name=\"neighbors*\" method=\"table\">\nNeighbor        V    AS   MsgRcvd   MsgSent   InQ  OutQ  Up/Down  St/PfxRcd\n{{ neighbor_address | IP }}        {{ version | DIGIT }}    {{ remote_as | DIGIT }}   {{ msg_rcvd | DIGIT }}   {{ msg_sent | DIGIT }}   {{ in_queue | DIGIT }}  {{ out_queue | DIGIT }}  {{ uptime }}  {{ state_pfx }}\n</group>",
    "field-mappings": "{\"neighbor_address\": \"neighbor-address\", \"remote_as\": \"remote-as\", \"state_pfx\": \"state\"}"
  }'
```

Note: In the JSON body, newlines in `cli-template` are encoded as `\n` and quotes as `\"`. The `cli-commands` field IS a JSON-encoded string array, but `cli-template` is a plain string.

## How TTP Results Map to Fields

1. TTP parses CLI output into structured records (list of maps)
2. The key source is resolved: the TTP variable that `field-mappings` maps onto the model's
   `is-key` field
...
Example: TTP produces `{"neighbor_address": "10.0.0.1", "remote_as": 65001}`, field-mappings is
`{"neighbor_address": "neighbor-address", "remote_as": "remote-as"}`, and `neighbor-address` is
the model's key field → the record is `{"neighbor-address": "10.0.0.1", "remote-as": 65001}`
keyed by `10.0.0.1`.

**The template must capture the key source as a variable.** If the TTP variable feeding the key
field never appears in the template, every record is dropped and the test returns zero rows —
the most common cause of an empty CLI result.
